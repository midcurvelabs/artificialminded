...
|---|---|---|---|---|
| **FRONTIER — CLOSED** | | | | |
| **GPT-5.5 (xhigh)** | TBD | **11.30** [^1] | 400K (est) | Released Apr 23, 2026. AA Intelligence Index 60 (leader). Input not yet on OpenRouter. [^2] |
| Claude Opus 4.7 | 5.00 | 25.00 [^3] | 1M | New tokenizer ~35% more tokens vs 4.6 [^4] |
| Claude Sonnet 4.6 | 3.00 | 15.00 [^3] | 1M | Default workhorse for most shops; #1 on OpenRouter usage (1.38T tokens/wk) [^5] |
| Claude Haiku 4.5 | 1.00 | 5.00 [^3] | 200K | Best quality/$ in cheap-closed tier |
| GPT-5.4 (standard) | 2.50 | 15.00 [^2] | 1.05M | Confirmed via OpenRouter API |
| GPT-5.4 Pro | 30.00 | 180.00 [^6] | 400K | Reasoning-heavy tier |
| GPT-5.4 Mini | ~0.40 | ~1.60 [^6] | 400K | |
| GPT-5 (original) | 0.63 | 5.00 [^7] | 200K | Cheaper, slightly older |
| GPT-4.1 | ~2.00 | ~8.00 confidence: medium | 1M | Legacy, being phased down |
| o4-mini | ~1.10 | ~4.40 confidence: medium | 200K | Reasoning, small |
| GPT-5 Nano | 0.05 | 0.40 [^8] | 128K | Dirt-cheap tier |
| Gemini 2.5 Pro | 1.00 | 10.00 [^9] | 1M | 2× input >200K tokens [^9] |
| Gemini 2.5 Pro (>200K ctx) | 2.50 | 15.00 [^9] | Long-context uplift | |
| Gemini 2.5 Flash | ~0.15 | ~0.60 confidence: medium | 1M | Cheap multimodal |
| Gemini 3.1 Pro Preview | 2.00 | 12.00 [^2] | 1.05M | Confirmed via OpenRouter |
| Grok 4 | 3.00 | 15.00 [^10] | 256K | |
| Grok 4.1 Fast | 0.20 | 0.50 [^10] | 256K | |
| **OPEN WEIGHTS (via API)** | | | | |
| DeepSeek V4 | 0.30 | 0.50 [^11] | 128K | Flagship, Mar 2026 |
| DeepSeek V3.2 | 0.14 | 0.28 [^12] | 128K | Cheapest frontier-adjacent |
| DeepSeek R1 (reasoning) | 0.55 | 2.19 [^13] | 128K | Reasoning tier |
...
| Kimi K2.6 | 0.74 | 4.66 [^2] | 256K | Top open-weights; confirmed via OpenRouter |
...
| Feature | Impact |
|---|---|
| **1M+ context at flat price** | Claude Opus 4.7, Sonnet 4.6 [^3]; Gemini 2.5 Pro **doubles** above 200K |
| **10M context** | Llama 4 Scout (unique; useful for full-repo work) [^16] |
...
|---|---|---|
| **Anthropic** | Up to **90%** on prompt cache reads [^3] | 50% batch |
| **OpenAI** | ~90% on cached input (e.g. GPT-5.2 $1.75→$0.175) [^17] | 50% batch |
...
$2.00 in     Gemini 3.1 Pro
$2.50 in     GPT-5.4 / Gemini long-ctx
$3.00 in     Sonnet 4.6 / Grok 4
$5.00 in     Opus 4.7
~$11 out     GPT-5.5 (input TBD; premium tier) [^1]
...
- GPT-5.5 input pricing not yet on OpenRouter; only output ($11.3) is published via Artificial Analysis.
...
Primary pricing/benchmark sources used to keep this grid current:
- **Artificial Analysis** — <https://artificialanalysis.ai/> (Intelligence Index, composite benchmarks, premium-tier pricing)
- **OpenRouter Models API** — <https://openrouter.ai/api/v1/models> (authoritative input/output prices + context length)
- **OpenRouter Rankings** — <https://openrouter.ai/rankings> (actual developer usage by tokens — signals what's being adopted vs. just announced)
- Vendor pricing pages (linked per row) for ground truth; aggregators used for cross-check.
